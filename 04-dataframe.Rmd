...
Un _data frame_ es una tabla de datos formada por columnas que pueden ser distinto tipo. Cada columna tiene asociado un nombre asociado. La función `data.frame()` permite especifar los vectores que formarán las columnas, y el nombre de cada una de ellas.
...
Por defecto, las cadenas de caracteres se convierten en factores. Si se quiere deshabilitar este comportamiento es preciso emplear el parámetro `stringAsFactors=FALSE`, e indicar explícitamente que columnas son factores.
...
Es habitual que las tablas que se almacenen contenga mucha información, de manera que visualizar todo el contenido, aunque sea de manera parcial, puede ser tedioso. Las funciones `head()` y `tail()` permiten inspeccionar los primeros y últimos elementos de la tabla.
...
Un _data frame_ se indexa de manera similar a un vector añadiendo un índice más poder referenciar tanto la fila como la columna. El siguiente ejemplo obtiene el valor de la celda que identifica por la fila 1 y la columan 2.
